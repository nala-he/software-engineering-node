...
class DislikeController {
    constructor() {
        ...
        this.findTuitsUserDisliked = (req, res) => {
            const uid = req.params.uid;
            const profile = req.session['profile'];
            const userId = uid === "me" && profile ?
                profile._id : uid;
            DislikeController.dislikeDao.findTuitsUserDisliked(userId)
                .then(dislikes => {
                const dislikesNonNullTuits = dislikes.filter(dislike => dislike.dislikedTuit);
                const tuitsFromDislikes = dislikesNonNullTuits.map(dislike => dislike.dislikedTuit);
